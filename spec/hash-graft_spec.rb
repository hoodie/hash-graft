...
describe Array do
  ...
  before(:each) do
    ...
    @example_1_fixed = @example_1.clone.freeze
    @example_2_fixed = @example_2.clone.freeze
    ...
    it "grafts nil onto Array" do
      a = [1,2,3,4]
      a_freeze = a.clone.freeze
      b = []
      b_freeze = b.clone.freeze

      expect(a.graft(nil)).to eq a_freeze
      expect(b.graft(nil)).to eq b_freeze
    end
end
...
describe Hash do
  ...
  before(:each) do
    ...
    it "grafts onto empty Hashes" do
      expect({}.graft @example_1).to eq @example_1_fixed
    end

    it "grafts empty Hash onto Hash" do
      expect(@example_1.graft({})).to eq @example_1_fixed
    end

    it "grafts nil onto Hash" do
      expect(@example_1.graft(nil)).to eq @example_1_fixed
    end
